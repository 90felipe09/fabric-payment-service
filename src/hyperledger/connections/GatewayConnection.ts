...
import { MICROPAYMENT_CHAINCODE, PAYMENT_CHANNEL } from '../config';
...
export class GatewayConnection {
    ...
    private connectToPeerGateway = async(): Promise<Gateway> => {
        if (!this.peerGateway) {
            ...
            await gateway.connect({}, gatewayOptions);
    ...
    public getMicropaymentChaincodeReference = async (): Promise<Contract> => {
        await this.connectToPeerGateway();
        await this.getChannelNetwork(PAYMENT_CHANNEL);
        const chaincode = await this.getChaincodeReference(MICROPAYMENT_CHAINCODE);
        return chaincode;
    }
